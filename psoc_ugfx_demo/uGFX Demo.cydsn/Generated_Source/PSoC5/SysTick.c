...
void SysTick_Start(void) 
{
    ...
    SysTick_CLKEN |= SysTick_CLKEN_MASK;
	SysTick_CLKSTBY |= SysTick_CLKSTBY_MASK;
}
...
void SysTick_Stop(void) 
{
    ...
    SysTick_CLKEN &= (uint8)(~SysTick_CLKEN_MASK);
	SysTick_CLKSTBY &= (uint8)(~SysTick_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: SysTick_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((SysTick_CLKEN & SysTick_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = SysTick_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = SysTick_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(SysTick_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(SysTick_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: SysTick_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: SysTick_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = SysTick_GetSourceRegister();
    uint16 oldDivider = SysTick_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = SysTick_CLKEN & SysTick_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((SysTick_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: SysTick_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(SysTick_DIV_PTR);
}


/*******************************************************************************
* Function Name: SysTick_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    SysTick_MOD_SRC |= modeBitMask & (uint8)SysTick_MODE_MASK;
}


/*******************************************************************************
* Function Name: SysTick_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    SysTick_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(SysTick_MODE_MASK));
}


/*******************************************************************************
* Function Name: SysTick_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return SysTick_MOD_SRC & (uint8)(SysTick_MODE_MASK);
}


/*******************************************************************************
* Function Name: SysTick_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = SysTick_GetDividerRegister();
    uint8 oldSrc = SysTick_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        SysTick_MOD_SRC |= CYCLK_SSS;
        SysTick_MOD_SRC =
            (SysTick_MOD_SRC & (uint8)(~SysTick_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: SysTick_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return SysTick_MOD_SRC & SysTick_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: SysTick_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    SysTick_PHASE = clkPhase & SysTick_PHASE_MASK;
}


/*******************************************************************************
* Function Name: SysTick_GetPhase
********************************************************************************
*
* Summary:
...
{
    return SysTick_PHASE & SysTick_PHASE_MASK;
}
